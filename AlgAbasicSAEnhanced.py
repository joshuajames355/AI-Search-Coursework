...
#represent state as an array of city indices, first element is the intial city.
...
def simulatedAnnealing(initialCity, distanceMatrix, numCities):
    NUM_STEPS = 100
    ITERATIONS_PER_STEP = 10000

    state = greedy(initialCity, distanceMatrix, numCities)
    score = getTourLength(distanceMatrix, numCities, state)
    
    for k in range(NUM_STEPS):
        temp = NUM_STEPS/(k+1)
        print("\rIteration: {} Temp: {} Best Score: {}".format(k, temp, score), end="")
        #sys.stdout.write("\033[K") 
        for _ in range(ITERATIONS_PER_STEP):
            newState = randomPermutation(state)
            newScore = getTourLength(distanceMatrix, numCities, newState)
            delta = score - newScore
